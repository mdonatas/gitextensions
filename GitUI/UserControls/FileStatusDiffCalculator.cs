...
namespace GitUI
{
    public sealed partial class FileStatusDiffCalculator
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            HashSet<string> sameBaseToAandB = allBaseToB.Intersect(allBaseToA, comparer).Except(allAtoB, comparer).Select(i => i.Name).ToHashSet();
            HashSet<string> onlyA = allBaseToA.Except(allBaseToB, comparer).Select(i => i.Name).ToHashSet();
            HashSet<string> onlyB = allBaseToB.Except(allBaseToA, comparer).Select(i => i.Name).ToHashSet();
